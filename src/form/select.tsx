...
export class Select extends Input<{ addDefaultBlankToItems?: boolean | string, items: Item[], readOnly?: boolean, }>{
    ...
    render(props?: any, state?: any) {
        ...
        return <div class="form-group">
            ...
            <div class={"select-style " + this.inputClass}>
                <select {...attributes} disabled={props.disabled} required={props.required} onChange={this.getOnChange()} name={props.name} class={this.inputClass}>
                    {items.map(x => <option selected={this.isSelected(x.value)} value={x.value}>{x.name}</option>)}
                </select>
            </div>
        </div>
    }
